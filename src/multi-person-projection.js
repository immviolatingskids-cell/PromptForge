...
import { PROJECTION_VERSION, versionMetadata } from "./versioning.js";
...
export function resolveMultiPersonProjection({ scene = {}, personas = [], project = null, cast = null, group = null, relationships = [], location = null, settings = {} } = {}) {
  ...
  if (!available.length) return { schemaVersion: MULTI_PERSON_PROJECTION_VERSION, versionMetadata: versionMetadata({ projection: PROJECTION_VERSION }), status: "empty", participants: [], warnings: ["No available participant references"], sourceTrace: [] };
  ...
  return { schemaVersion: MULTI_PERSON_PROJECTION_VERSION, versionMetadata: versionMetadata({ projection: PROJECTION_VERSION }), status: missing.length ? "degraded" : "ready", seed: scene.seed, density, target, focusCharacter: { id: idOf(focus), name: nameOf(focus) }, participants: participantRecords, arrangement, summary: `${nameOf(focus)} anchors a shared scene with ${participantRecords.filter(item => item.id !== idOf(focus)).map(item => item.name).join(", ") || "no supporting participant"} at ${context}.`, prompt, negativePrompt, sourceTrace, warnings: missing.map(id => `Missing participant reference: ${id}`), package: { schemaVersion: MULTI_PERSON_PROJECTION_VERSION, versionMetadata: versionMetadata({ projection: PROJECTION_VERSION }), sceneId: scene.id, participantRefs: participantIds.map(id => ({ type: "persona", id })), focusPersonaRef: { type: "persona", id: idOf(focus) }, relationshipRefs: evidence.map(item => ({ type: "relationship", id: item.id })), settings: { density, target, ...settings }, sourceTrace } };
}
