...
export function initProductShell(bridge) {
  ...
  function projectDetailView(id) {
    ...
    const assembly = assembleProjectContext({ project, personas: personas().map(item => item.persona), casts: bridge.castStore?.all?.() || [], groups: bridge.groupStore?.all?.() || [], relationships: relationshipStore?.all?.() || [], locations, scenes, settings: { density: "standard" } });
  ...
  document.addEventListener("click", async event => {
    ...
    if (assemblyButton) { const project = projectStore.open(assemblyButton.dataset.id); if (project) { const result = assembleProjectContext({ project, personas: personas().map(item => item.persona), casts: bridge.castStore?.all?.() || [], groups: bridge.groupStore?.all?.() || [], relationships: relationshipStore?.all?.() || [], locations: bridge.locationStore?.all?.() || [], scenes: project.sceneRefs.map(ref => sceneStore.open(ref.id)).filter(Boolean) }); const markdown = assemblyButton.dataset.projectAssembly === "markdown"; downloadJson(`${project.id}.promptforge-context.${markdown ? "md" : "json"}`, markdown ? projectContextMarkdown(result) : JSON.stringify(result.package, null, 2)); } return; }
