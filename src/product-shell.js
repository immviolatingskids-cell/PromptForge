...
export function initProductShell(bridge) {
  ...
  function projectDetailView(id) {
    ...
    view.insertAdjacentHTML("beforeend", `<section class="pf-section"><div class="pf-section-heading"><div><p class="pf-kicker">Scene Forge</p><h2>Project scenes</h2></div><button class="pf-primary" data-product-action="project-scene" data-id="${escapeHtml(id)}">New scene</button></div>${missingSceneRefs.length ? `<p class="warning">${missingSceneRefs.length} scene reference${missingSceneRefs.length === 1 ? " is" : "s are"} missing. IDs: ${escapeHtml(missingSceneRefs.map(ref => ref.id).join(", "))}</p>` : ""}<div class="pf-list">${scenes.map(scene => `<div class="pf-project-searchable" data-project-search="${escapeHtml(`${scene.title} ${scene.premise} ${scene.requirements?.location || ""} ${scene.state?.needsReview ? "review" : ""}`)}">${sceneCard(scene, personById(scene.focusPersonaRef?.id || scene.characterRef?.id))}</div>`).join("") || '<div class="pf-compact-empty">No scenes yet. Project context will be inherited by default.</div>'}</div></section>`);
