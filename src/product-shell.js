...
export function initProductShell(bridge) {
  ...
  function projectDetailView(id) {
    ...
    const projections = scenes.map(scene => scene.output?.package?.multiPersonProjection || scene.resolved?.multiPersonProjection).filter(Boolean).map((item, index) => ({ ...item, sceneId: scenes[index]?.id }));
    const assembly = assembleProjectContext({ project, personas: personas().map(item => item.persona), casts: bridge.castStore?.all?.() || [], groups: bridge.groupStore?.all?.() || [], relationships: relationshipStore?.all?.() || [], locations, scenes, projections, settings: { ...assemblySettings, density: assemblySettings.density || "standard" } });
