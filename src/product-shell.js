...
export function initProductShell(bridge) {
  ...
  function projectDetailView(id) {
    ...
    const projectProjections = scenes.map(scene => ({ sceneId: scene.id, ...(scene.output?.package?.multiPersonProjection || scene.resolved?.multiPersonProjection || {}) })).filter(item => item.status || item.warnings?.length);
    const coherence = auditProjectCoherence({ project, personas: personas().map(item => item.persona), casts: bridge.castStore?.all?.() || [], groups: bridge.groupStore?.all?.() || [], relationships: relationshipStore?.all?.() || [], locations, scenes, projections: projectProjections, assembly });
    const projectCasts = project.castRefs?.length || 0, projectGroups = project.groupRefs?.length || 0, projectRelationships = project.relationshipRefs?.length || 0, projectLocations = project.locationRefs?.length || 0;
    view.innerHTML = `<div class="pf-detail-actions"><a href="#projects" class="pf-back">← All projects</a><div><button data-product-action="duplicate-project" data-id="${escapeHtml(id)}">Duplicate</button><button data-product-action="export-project" data-id="${escapeHtml(id)}">Export JSON</button><button data-product-action="delete-project" data-id="${escapeHtml(id)}">Delete</button><button class="pf-primary" data-product-action="project-character" data-id="${escapeHtml(id)}">Generate persona</button></div></div><section class="pf-project-hero pf-surface"><div><p class="pf-kicker">PromptForge → Project → Personas</p><h2>${escapeHtml(project.name)}</h2><p>${escapeHtml(project.description || project.context.premise || "A shared creative context.")}</p>${project.state.needsReview ? '<p class="warning">Project context changed. Inheriting members are marked for review; explicit overrides were preserved.</p>' : ""}</div><div class="pf-project-stats"><span><strong>${characters.length}</strong>Personas</span><span><strong>${projectCasts + projectGroups}</strong>Casts / groups</span><span><strong>${projectRelationships}</strong>Relationships</span><span><strong>${projectLocations}</strong>Locations</span><span><strong>${scenes.length}</strong>Scenes</span><span><strong>${project.state.revision}</strong>Revision</span><span><strong>${escapeHtml(dateLabel(project.updatedAt))}</strong>Updated</span></div></section>
    <div class="pf-project-columns"><section class="pf-surface pf-profile-panel"><div class="pf-section-heading"><div><p class="pf-kicker">Members</p><h2>Personas</h2></div></div><div class="pf-inline-form"><label class="pf-search"><span aria-hidden="true">⌕</span><input id="project-detail-search" type="search" placeholder="Filter this project" aria-label="Filter project entities"></label><label><span class="pf-sr-only">Review filter</span><select id="project-detail-filter" aria-label="Filter project status"><option value="all">All items</option><option value="review">Needs review</option><option value="archived">Archived</option></select></label></div>${characters.length ? `<div class="pf-list">${characters.map(persona => { const member=project.memberState[persona.meta.persona_id]; const link=persona.extensions?.promptforge?.projectContexts?.[id]; const inherited=Object.keys(link?.inherited||{}); const overrides=Object.keys(link?.overrides||{}); return `<article class="pf-scene-card pf-project-searchable" data-project-status="${member?.needsReview ? "review" : "active"}" data-project-search="${escapeHtml(`${persona.origin.name} ${inherited.join(" ")} ${overrides.join(" ")} ${member?.needsReview ? "review" : ""}`)}"><div><h3>${escapeHtml(persona.origin.name)}</h3><p>${inherited.length ? `Inherits ${escapeHtml(inherited.join(", "))}` : "Independent context"}${overrides.length ? ` · Overrides ${escapeHtml(overrides.join(", "))}` : ""}${member?.needsReview ? " · Review needed" : ""}</p></div><div><button data-product-action="open-character" data-id="${escapeHtml(persona.meta.persona_id)}">Open</button><button data-product-action="remove-project-persona" data-project-id="${escapeHtml(id)}" data-persona-id="${escapeHtml(persona.meta.persona_id)}">Remove</button></div></article>`; }).join("")}</div>` : '<p class="pf-muted">No personas attached yet.</p>'}${available.length ? `<form id="project-add-character" data-project-id="${escapeHtml(id)}" class="pf-inline-form"><select name="personaId" aria-label="Persona to add">${available.map(item => `<option value="${escapeHtml(item.persona.meta.persona_id)}">${escapeHtml(item.persona.origin.name)}</option>`).join("")}</select><button type="submit">Add existing persona</button></form>` : ""}</section>
    ...
    view.insertAdjacentHTML("beforeend", `<section class="pf-section"><div class="pf-section-heading"><div><p class="pf-kicker">Scene Forge</p><h2>Project scenes</h2></div><button class="pf-primary" data-product-action="project-scene" data-id="${escapeHtml(id)}">New scene</button></div>${missingSceneRefs.length ? `<p class="warning">${missingSceneRefs.length} scene reference${missingSceneRefs.length === 1 ? " is" : "s are"} missing. IDs: ${escapeHtml(missingSceneRefs.map(ref => ref.id).join(", "))}</p>` : ""}<div class="pf-list">${scenes.map(scene => `<div class="pf-project-searchable" data-project-status="${scene.state?.needsReview || scene.state?.stale ? "review" : "active"}" data-project-search="${escapeHtml(`${scene.title} ${scene.premise} ${scene.requirements?.location || ""} ${scene.state?.needsReview ? "review" : ""}`)}">${sceneCard(scene, personById(scene.focusPersonaRef?.id || scene.characterRef?.id))}</div>`).join("") || '<div class="pf-compact-empty">No scenes yet. Project context will be inherited by default.</div>'}</div></section>`);
    document.querySelectorAll(".pf-project-columns .pf-profile-panel").forEach(section => { if (section.querySelector("h2")?.textContent === "Relationships") section.querySelectorAll("article").forEach(card => { card.classList.add("pf-project-searchable"); card.dataset.projectStatus = "active"; card.dataset.projectSearch = card.textContent; }); });
    view.insertAdjacentHTML("beforeend", `<section class="pf-section pf-coherence-panel"><div class="pf-section-heading"><div><p class="pf-kicker">Graph health</p><h2>Coherence and review</h2></div><span class="pf-status-chip">${escapeHtml(coherence.status)}</span></div><p>${coherence.counts.error} errors · ${coherence.counts.warning} warnings</p>${coherence.issues.length ? `<ul>${coherence.issues.map(item => `<li><strong>${escapeHtml(item.severity)}</strong> ${escapeHtml(item.message)} <small>${escapeHtml(item.recovery)}</small></li>`).join("")}</ul>` : '<p class="pf-muted">All current project references resolve cleanly.</p>'}</section><section class="pf-section pf-activity-panel"><div class="pf-section-heading"><div><p class="pf-kicker">Recent activity</p><h2>Project history</h2></div><span class="pf-muted">${project.workspace.activity.length} recorded</span></div>${project.workspace.activity.length ? `<ol>${[...project.workspace.activity].reverse().slice(0, 12).map(item => `<li><strong>${escapeHtml(humanize(item.type))}</strong><span>${escapeHtml(item.detail || "Project workspace updated")}</span><small>${escapeHtml(dateLabel(item.at))}</small></li>`).join("")}</ol>` : '<p class="pf-muted">Meaningful project activity will appear here. Legacy records are not backfilled.</p>'}</section>`);
  ...
  root.addEventListener("input", event => {
    ...
    if (event.target.matches('#project-detail-search')) {
      const query = event.target.value.trim().toLowerCase().slice(0, 120);
      const status = document.querySelector('#project-detail-filter')?.value || 'all';
      document.querySelectorAll('.pf-project-searchable').forEach(item => item.hidden = Boolean((query && !item.dataset.projectSearch.toLowerCase().includes(query)) || (status !== 'all' && item.dataset.projectStatus !== status)));
    }
  });
  root.addEventListener('change', event => {
    if (!event.target.matches('#project-detail-filter')) return;
    const query = document.querySelector('#project-detail-search')?.value.trim().toLowerCase().slice(0, 120) || '';
  ...
  document.addEventListener("submit", event => {
    ...
    if (event.target.id === "scene-form") {
      ...
      if (project) { projectStore.addScene(project.id, scene.id); projectStore.recordActivity(project.id, event.target.dataset.sceneId ? "scene.edited" : "scene.created", scene.title); }
    ...
    if (event.target.id === "project-assembly-settings") {
      ...
      const current = project.workspace?.contextAssembly || {}; projectStore.update(project.id, { workspace: { ...project.workspace, contextAssembly: { ...current, density: values.density, purpose: values.purpose, notes: values.notes, selected } }, activityType: "context.changed" }); navigate();
    }
