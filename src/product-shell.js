...
export function initProductShell(bridge) {
  ...
  root.addEventListener("input", event => {
    ...
    if (event.target.matches('#project-detail-search')) {
      ...
      document.querySelectorAll('.pf-project-searchable').forEach(item => item.hidden = Boolean(query && !item.dataset.projectSearch.toLowerCase().includes(query)));
    }
  });
