...
const activity = (type, detail = "") => ({ type: clean(type, 80), detail: clean(detail, 240), at: now() });
...
export function normalizeProject(raw = {}) {
  ...
  return {
    ...
    versionMetadata: versionMetadata({ application: project.versionMetadata?.application || APPLICATION_VERSION, project: 1 }),
    workspace: { favorite: project.workspace?.favorite === true, archived: project.workspace?.archived === true, tags: [...new Set((Array.isArray(project.workspace?.tags) ? project.workspace.tags : []).map(value => clean(value, 60)).filter(Boolean))].slice(0, 20), activeSection: clean(project.workspace?.activeSection, 80) || "overview", selectedSources: structuredClone(object(project.workspace?.selectedSources)), reviewFilters: structuredClone(object(project.workspace?.reviewFilters)), collapsedPanels: structuredClone(object(project.workspace?.collapsedPanels)), lastOpenedScene: clean(project.workspace?.lastOpenedScene, 160) || null, lastOpenedParticipant: clean(project.workspace?.lastOpenedParticipant, 160) || null, activity: Array.isArray(project.workspace?.activity) ? project.workspace.activity.slice(-100).map(item => ({ type: clean(item?.type, 80), detail: clean(item?.detail, 240), at: typeof item?.at === "string" ? item.at : null })) : [], lastOpenedAt: typeof project.workspace?.lastOpenedAt === "string" ? project.workspace.lastOpenedAt : null, contextAssembly: { density: ["compact", "standard", "detailed"].includes(project.workspace?.contextAssembly?.density) ? project.workspace.contextAssembly.density : "standard", purpose: clean(project.workspace?.contextAssembly?.purpose, 120), notes: clean(project.workspace?.contextAssembly?.notes, 4000), selected: structuredClone(object(project.workspace?.contextAssembly?.selected)) } },
...
export class ProjectStore {
  ...
  recordActivity(projectId, type, detail = "") { const project = this.open(projectId); if (!project) return null; const events = [...project.workspace.activity, activity(type, detail)].slice(-100); return this.update(projectId, { workspace: { ...project.workspace, activity: events }, activityType: type }); }
  ...
  browserData() { return { projects: this.all(), projectSchemaVersion: PROJECT_SCHEMA_VERSION, versionMetadata: versionMetadata({ project: 1 }) }; }
